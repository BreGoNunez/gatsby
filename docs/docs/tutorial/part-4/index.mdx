...
**Note:** In this example, as we already have an Seo component to handle the title for us, we don't need to retrieve the title again using the `data` prop; our Seo component already did that for us earlier on this tutorial, so no need to copy the code above if you've been following the tutorial and want to use the Seo component here.
